...
class DenoisingAutoencoder:
    ...
    def __init__(self, train_data, train_label, validation_data, validation_label, test_data, test_label):
        print("in denoising initialization ")
    ...
    def train(self,batches_x, batches_y, n_batches):
        print(" in train ")
        net_dims = [784, 1024, 784]
        num_iterations = 1000
        learning_rate = 0.1
        decay_rate = 0.01
        corrupted_images = self.add_noise(batches_x)
        network_type = 'DAE'
        costs, validation_costs, parameters = \
            multi_layer_network(self.train_data, self.train_label, self.validation_data, self.validation_label,
                                net_dims, network_type, batches_x, batches_y, n_batches, corrupted_input=corrupted_images, num_iterations=num_iterations,
